...
#import "DLControlView.h"
#import "DLPauseOrPlayView.h"
//横竖屏的时候过渡动画时间，设置为0.0则是无动画
#define kTransitionTime 0.2
//填充模式枚举值
typedef NS_ENUM(NSInteger,DLLayerVideoGravity){
    DLLayerVideoGravityResizeAspect,
    DLLayerVideoGravityResizeAspectFill,
    DLLayerVideoGravityResize,
};
//播放状态枚举值
typedef NS_ENUM(NSInteger,DLPlayerStatus){
    DLPlayerStatusFailed,
    DLPlayerStatusReadyToPlay,
    DLPlayerStatusUnknown,
    DLPlayerStatusBuffering,
    DLPlayerStatusPlaying,
    DLPlayerStatusStopped,
};
@interface DLPlayer : UIView<DLControlViewDelegate,DLPauseOrPlayViewDelegate,UIGestureRecognizerDelegate>{
...
@property (nonatomic,assign) DLLayerVideoGravity mode;
//是否正在播放
