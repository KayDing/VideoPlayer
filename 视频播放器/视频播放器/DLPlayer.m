...
//底部控制视图
@property (nonatomic,strong) DLControlView *controlView;
//暂停和播放视图
@property (nonatomic,strong) DLPauseOrPlayView *pauseOrPlayView;
//原始约束
...
-(void)setMode:(DLLayerVideoGravity)mode{
    switch (mode) {
        case DLLayerVideoGravityResizeAspect:
            self.playerLayer.videoGravity = AVLayerVideoGravityResizeAspect;
            break;
        case DLLayerVideoGravityResizeAspectFill:
            self.playerLayer.videoGravity = AVLayerVideoGravityResizeAspectFill;
            break;
        case DLLayerVideoGravityResize:
...
-(void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context{
    if ([keyPath isEqualToString:@"status"]) {
        ...
        switch (itemStatus) {
            ...
            {
                _status = DLPlayerStatusUnknown;
                NSLog(@"AVPlayerItemStatusUnknown");
            }
                break;
            case AVPlayerItemStatusReadyToPlay:
            {
                _status = DLPlayerStatusReadyToPlay;
                NSLog(@"AVPlayerItemStatusReadyToPlay");
            }
                break;
            case AVPlayerItemStatusFailed:
            {
                _status = DLPlayerStatusFailed;
        ...
        _status = DLPlayerStatusBuffering;
        ...
        if ([[change objectForKey:NSKeyValueChangeNewKey]integerValue]==0) {
            _isPlaying=false;
            _status = DLPlayerStatusPlaying;
        }else{
            _isPlaying=true;
            _status = DLPlayerStatusStopped;
        }
    }
...
//懒加载暂停或者播放视图
-(DLPauseOrPlayView *)pauseOrPlayView{
    if (!_pauseOrPlayView) {
        _pauseOrPlayView = [[DLPauseOrPlayView alloc]init];
...
//懒加载控制视图
-(DLControlView *)controlView{
    if (!_controlView) {
        _controlView = [[DLControlView alloc]init];
...
-(void)controlView:(DLControlView *)controlView pointSliderLocationWithCurrentValue:(CGFloat)value{
    count = 0;
...
-(void)controlView:(DLControlView *)controlView draggedPositionWithSlider:(UISlider *)slider{
    count = 0;
    CMTime pointTime = CMTimeMake(controlView.value * self.item.currentTime.timescale, self.item.currentTime.timescale);
    [self.item seekToTime:pointTime toleranceBefore:kCMTimeZero toleranceAfter:kCMTimeZero];
}
-(void)controlView:(DLControlView *)controlView withLargeButton:(UIButton *)button{
    count = 0;
...
-(BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch{
    if ([touch.view isKindOfClass:[DLControlView class]]) {
        return NO;
    }
    return YES;
}
